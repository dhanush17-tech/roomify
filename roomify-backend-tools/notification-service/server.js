  ...
  initializeApp({
    credential: cert(
      require("./roomify-f1a1c-firebase-adminsdk-46b0s-85ed61448c.json") || "{}"
    ),
  });
  ...
  const createNotificationConfig = (token, notification, data) => ({
    ...
    android: {
      ...
      notification: {
        ...
        channelId: data.type === NOTIFICATION_TYPES.CHAT 
          ? CHANNEL_IDS.CHAT 
          : CHANNEL_IDS.LISTING,
      },
    },
  ...
  const handleListingReportedNotification = (notification, data, recipientFCMToken) =>
    createNotificationConfig(
      recipientFCMToken,
      {
  ...
  const handleDocumentRequestNotification = (sender, documents, roomId, requestId, recipientFCMToken) =>
    createNotificationConfig(
      recipientFCMToken,
      {
    ...
    const handleDocumentSubmissionNotification = (sender, documents, roomId, requestId, recipientFCMToken) =>
    createNotificationConfig(
      recipientFCMToken,
      {
    ...
    try {
      ...
      switch (type) {
        case NOTIFICATION_TYPES.CHAT: {
          ...
          notificationPayload = handleChatNotification(message, sender, recipientFCMToken);
          break;
        }

        case NOTIFICATION_TYPES.LISTING_REPORTED: {
          const { notification, data } = req.body;
          notificationPayload = handleListingReportedNotification(notification, data, recipientFCMToken);
          break;
        }

        case NOTIFICATION_TYPES.LISTING_REMOVED: {
          const { listing } = req.body;
          notificationPayload = handleListingRemovedNotification(listing, recipientFCMToken);
          break;
        }
